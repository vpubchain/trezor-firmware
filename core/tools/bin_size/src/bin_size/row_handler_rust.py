...
class RustRow(CommonRow):
    ...
    def _get_module_and_function(self, symbol_name: str) -> tuple[str, str]:
        ...
        symbol_name = self._replace_dollar_encodings(symbol_name)
        ...
        if symbol_name.startswith("_"):
            symbol_name = self._get_real_symbol_from_alias(symbol_name)
        ...
        if symbol_name.startswith("trezor_lib::"):
            return self._resolve_trezorlib_symbol(symbol_name)
        else:
            return "", f"{symbol_name}()"

    def _resolve_trezorlib_symbol(self, symbol_name: str) -> tuple[str, str]:
    ...
    @staticmethod
    def _replace_dollar_encodings(symbol_name: str) -> str:
        REPLACEMENTS = {
            "$LT$": "<",
            "$GT$": ">",
            "$RF$": "&",
    ...
    @staticmethod
    def _get_real_symbol_from_alias(symbol_name: str) -> str:
