...
class MapFileIncluder(MapFileIncluderAPI):
    def add_info(
        ...
        for section in sections:
            map_symbol_sizes = self._get_symbol_sizes(map_file, section)
            map_symbols_we_miss = self._get_symbols_we_miss(
                row_data, map_symbol_sizes, section
            )
            added_size = self._add_map_file_info(
                row_data, map_symbol_sizes, map_symbols_we_miss, section
            )

            print(f"Added {added_size:_} bytes to {section} section from {map_file}")

            self._decrease_size_of_mysterious_section(row_data, section, added_size)

        return row_data

    @staticmethod
    def _get_symbol_sizes(map_file: str | Path, section: str) -> dict[str, int]:
    ...
    @staticmethod
    def _decrease_size_of_mysterious_section(
        ...
        for row in row_data:
            if row.symbol_name == section_symbol:
                row.size -= added_size
                print(
                    f"Decreasing the size of `{section_symbol}` by {added_size:_} bytes"
                )
                break
        else:
