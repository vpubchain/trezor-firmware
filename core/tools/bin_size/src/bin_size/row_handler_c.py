...
from .row_handler_common import CORE_DIR, HERE, CommonRow
...
class CRow(CommonRow):
    ...
    def _is_data(self, row: DataRow) -> bool:
        ...
        return self._is_data_cached(row.build_definition)

    @staticmethod
    @file_cache(HERE / "is_data_cache.json")
    def _is_data_cached(build_definition: str) -> bool:
        if not build_definition or ":" not in build_definition:
            return False

        def_file, line_num = build_definition.split(":")
        def_file = CORE_DIR / def_file
    ...
    @staticmethod
    def _find_at_least_something(symbol_name: str, dir_to_search: str | Path) -> str:
