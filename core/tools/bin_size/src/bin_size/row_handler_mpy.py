...
from .row_handler_common import CORE_DIR, INVALID_FILE_PREFIX, MPY_PREFIXES, CommonRow
...
class MicropythonRow(CommonRow):
    ...
    def _get_module_and_function(self, symbol_name: str) -> tuple[str, str]:
        ...
        module_name, is_valid = self._resolve_module(module_name)
        if not is_valid:
            module_name = f"{INVALID_FILE_PREFIX}{module_name}"
        else:
            func_name = self._resolve_function_name(func_name, module_name)

        return module_name, func_name

    def _resolve_function_name(self, func_name: str, module_name: str) -> str:
        # Deleting strange things at the end
        func_name = self._remove_strange_suffixes(func_name)

        if not func_name:
            return ""

        module_defs = self._get_module_object_definitions(module_name)
        return module_defs.resolve_symbol(func_name)

    @staticmethod
    def _remove_strange_suffixes(symbol_name: str) -> str:
        return re.sub(r"(_)?_lt_\w+_gt(_\d*)?", "", symbol_name)

    @staticmethod
    @lru_cache(maxsize=None)
    def _get_module_object_definitions(
        module_path: str | Path,
    ) -> ObjectDefinition:
        """Get all the functions and classes defined in a given module"""

        def _resolve_object(
            main_node: ast.Module
            | ast.ClassDef
            | ast.FunctionDef
            ...
            return ObjectDefinition(
                ...
                if file_name
                    ...
                    main_node.name
                    if not isinstance(main_node, ast.Module)
                    else "file_name"
                ),
                functions=functions,
                classes=classes,
        ...
        if not Path(CORE_DIR / module_path).is_file():
            return _default_object()

        with open(CORE_DIR / module_path) as file:
    ...
    @staticmethod
    @lru_cache(maxsize=None)
    def _resolve_module(module_name: str) -> tuple[str, bool]:
        ...
        for part in module_parts:
            ...
            if (CORE_DIR / possible_path).exists():
        ...
        if (
            not Path(CORE_DIR / file_path).exists()
        ...
        is_valid = Path(CORE_DIR / file_path).exists()
    ...
    def _get_definition(self, row: DataRow) -> str:
        ...
        module_defs = self._get_module_object_definitions(row.module_name)
