...
REAL_FILE = CORE_DIR / "build/firmware/firmware.elf"
...
def test_load_file():
    ...
    if not REAL_FILE.exists():
        pytest.fail(f"{REAL_FILE} not found")
    BS = get_bloaty_BS()
    BS.load_file(REAL_FILE)
    assert BS.get_len() > 0
...
def test_get_summary_data():
    ...
    BS = get_bloaty_BS().load_data(rows)
    assert BS._get_data_summary() == "SUMMARY: 3 rows, 404 bytes in total."
