...
from ..src.bin_size.source_definition_cache import SourceDefinitionCache
...
def test_get_file_hash():
    CACHE = SourceDefinitionCache()
    assert (
        CACHE._get_file_hash(CORE_DIR / "vendor/trezor-crypto/nist256p1.c")
        == "829a11e46fec13e5be5d787ce0c61e4a"
    )
    assert (
        CACHE._get_file_hash(CORE_DIR / "vendor/trezor-crypto/secp256k1.c")
...
def test_get_file_location_from_definition():
    ...
    ) == str(CORE_DIR / "vendor/trezor-crypto/nist256p1.c")
    assert CACHE._get_file_location_from_definition(
        "vendor/trezor-crypto/nist256p1.c"
    ) == str(CORE_DIR / "vendor/trezor-crypto/nist256p1.c")
    assert CACHE._get_file_location_from_definition(
        "embed/extmod/modtrezorui/loader_R.h:3"
    ) == str(CORE_DIR / "embed/extmod/modtrezorui/loader_R.h")
