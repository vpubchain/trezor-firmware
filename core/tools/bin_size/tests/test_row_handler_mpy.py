...
from ..src.bin_size.row_handler_mpy import MicropythonRow
...
def test_module_object_definitions_general():
    ...
    mod = MPR._get_module_object_definitions("src/apps/binance/sign_tx.py")
    ...
    mod = MPR._get_module_object_definitions("src/trezor/crypto/scripts.py")
    ...
    mod = MPR._get_module_object_definitions("src/trezor/loop.py")
    ...
    # Nested classes
    mod = MPR._get_module_object_definitions("src/trezor/ui/components/tt/info.py")
    ...
    # Nested functions
    mod = MPR._get_module_object_definitions("src/storage/cache.py")
...
def test_module_object_definitions_symbol_resolution():
    ...
    mod = MPR._get_module_object_definitions("src/apps/monero/xmr/bulletproof.py")
    assert (
...
def test_module_object_definitions_line_number():
    ...
    mod = MPR._get_module_object_definitions("src/storage/cache.py")
    assert mod.get_line_number("stored_async()") == 284
    mod = MPR._get_module_object_definitions("src/apps/monero/xmr/bulletproof.py")
    ...
    # Unexisting module
    mod = MPR._get_module_object_definitions("src/apps/monero/xmr/unexisting.py")
    assert mod.get_line_number("unexisting()") == 0
...
def test_resolve_function_name(symbol_part: str, module: str, func: str):
    assert MPR._resolve_function_name(symbol_part, module) == func
...
def test_remove_strange_suffixes(input: str, output: str):
    assert MPR._remove_strange_suffixes(input) == output
...
def test_resolve_module(symbol: str, module: str, is_valid: bool):
    assert MPR._resolve_module(symbol) == (module, is_valid)
