...
@click.option(
    "-fn", "--func-name", help="Check only specific function - e.g. `get_tx_keys`"
)
@click.option("-b", "--build", is_flag=True, help="Perform build")
@click.option(
    "-as", "--all-sections", is_flag=True, help="Get all sections, not just flash"
)
@click.option(
...
@click.option(
    "-na", "--no-aggregation", is_flag=True, help="Do not aggregate symbols together"
)
@click.option("-ns", "--no-sort", is_flag=True, help="Do not sort by size")
@click.option("-np", "--no-processing", is_flag=True, help="See just raw data")
def get(
    custom_file: str,
    output_file: str,
    app: str,
