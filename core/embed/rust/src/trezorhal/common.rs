...
pub fn __fatal_error(expr: &str, msg: &str, file: &str, line: i32, func: &str) -> ! {
...
pub trait UnwrapOrFatalError<T> {
    fn unwrap_or_fatal_error(self, expr: &str, msg: &str, file: &str, line: i32, func: &str) -> T;
}
...
macro_rules! unwrap {
    ($e:expr, $msg:expr) => {{
        ...
        $e.unwrap_or_fatal_error("unwrap failed", $msg, file!(), line!() as i32, function_name!())
    }};
    ($expr:expr) => {
        unwrap!($expr, "")
    };
...
macro_rules! ensure {
    ($what:expr, $error:expr) => {
        if !($what) {
            fatal_error!("ensure failed", $error);
        }
    };
}
...
macro_rules! fatal_error {
    ($expr:expr, $msg:expr) => {{
        crate::trezorhal::common::fatal_error(
            $expr,
            $msg,
            file!(),
            line!() as i32,
