...
def get_encoded_definitions_from_msg(msg: MessageType) -> Tuple[bytes | None, bytes | None, int | None, int | None]:
    ...
    return encoded_network_definition, encoded_token_definition, chain_id, token_address


class EthereumDefinitionParser:
    def __init__(self, definition_bytes: bytes) -> None:
        if len(definition_bytes) <= (2 + 1 + 4 + 64):
            raise wire.DataError("Received Ethereum definition is probably malformed (too few data).")

        self.format_version = int.from_bytes(definition_bytes[:2], byteorder='big')
        self.definition_type = definition_bytes[2]
        self.data_version = int.from_bytes(definition_bytes[3:7], byteorder='big')
        self.payload = definition_bytes[7:-64]
...
def decode_definition(
    ...
    if parsed_definition.format_version < MIN_FORMAT_VERSION:
        raise wire.DataError("Used Ethereum definition format version too low.")
