...
on publicly accessible website - on `data.trezor.io`. To ensure, that the definitions send to device are genuine, every definition
generated is signed and checked in FW.
Every definition (network/token) is encoded and has special format:
1. prefix:
   1. part: format version of the definition (unsigned integer, 2 bytes)
