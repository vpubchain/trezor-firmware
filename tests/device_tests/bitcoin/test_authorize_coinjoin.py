...
def test_wrong_account_type(client: Client):
    ...
    with pytest.raises(TrezorFailure, match="Forbidden path"):
        btc.authorize_coinjoin(
            **params,
    ...
    # Ensure that Trezor accepts CoinJoin authorizations only for Taproot scripts.
    with pytest.raises(TrezorFailure, match="Forbidden path"):
        btc.authorize_coinjoin(
            **params,
            n=parse_path("m/10025h/1h/0h/1h"),
...
def test_get_public_key(client: Client):
    ...
    # Ensure that user needs to confirm access to CoinJoin XPUB when preauthorization is not used.
    with client:
        client.set_expected_responses(
            [messages.ButtonRequest(code=B.Other), messages.PublicKey]
        )
        resp = btc.get_public_node(client, parse_path("m/10025h/1h/0h/1h"))
        assert resp.xpub == expected_xpub

    # Ensure that user does not need to confirm access to CoinJoin XPUB when preauthorization is used.
    with client:
        client.set_expected_responses(
            [messages.PreauthorizedRequest, messages.PublicKey]
        )
        resp = btc.get_public_node(
            client, parse_path("m/10025h/1h/0h/1h"), preauthorized=True
        )
        assert resp.xpub == expected_xpub


def test_get_address(client: Client):
    # Ensure that the SLIP-0025 external chain is freely accessible.
    ...
    # Ensure that the SLIP-0025 internal chain is inaccessible.
